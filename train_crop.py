...
class ClipDramaDataset(Dataset):
...
def train_and_validate(train_dataset: ClipDramaDataset, val_dataset: ClipDramaDataset, model: ClipCaptionDramaModel, 
                       args, lr: float = 2e-5, warmup_steps: int = 5000, output_dir: str = ".", output_prefix: str = ""):
...
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data_train', default='./data/drama/woo_split_ViT-B_32_crop_train.pkl')
    parser.add_argument('--data_val', default='./data/drama/woo_split_ViT-B_32_crop_val.pkl')
    parser.add_argument('--out_dir', default='./drama_train/crop/')
    ...
    dataset_train = ClipDramaDataset(args.data_train, prefix_length, normalize_prefix=args.normalize_prefix)
    dataset_val = ClipDramaDataset(args.data_val, prefix_length, normalize_prefix=args.normalize_prefix)
    ...
    if args.only_prefix:
        ...
        model = ClipCaptionDramaModel(prefix_length, clip_length=args.prefix_length_clip, prefix_size=prefix_dim,
                                  num_layers=args.num_layers, mapping_type=args.mapping_type)
        print("Train both prefix and GPT")
        sys.stdout.flush()
    train_and_validate(dataset_train, dataset_val, model, args, output_dir=args.out_dir, output_prefix=args.prefix)
